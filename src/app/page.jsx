...
const MatrixRain = () => {
    ...
    useEffect(() => {
            ...
            if (net && video && video.readyState >= 3) {
                try {
                    const newSegmentation = await net.segmentPerson(video, {
                        flipHorizontal: false, // Set to false to match the non-mirrored video
        ...
        const draw = () => {
            ...
            // --- LEFT HALF: Original Video (Not Mirrored) ---
            // Draw the entire video feed first to cover the whole canvas
            ctx.drawImage(video, 0, 0, dimensions.width, dimensions.height);

            // --- RIGHT HALF: Matrix Effect ---
            // Draw a black background only over the right half, on top of the video
            ctx.fillStyle = 'black';
            ctx.fillRect(halfWidth, 0, halfWidth, dimensions.height);
            
            const columns = Math.floor(halfWidth / fontSize);
            ...
            for (let i = 0; i < columns; i++) {
                ...
                for (let j = 0; j < rows; j++) {
                    // Calculate x position on the right half of the canvas
                    const x = halfWidth + (i * fontSize);
                    const y = j * fontSize;

                    // Check for collision with the live person silhouette
                    let isPerson = false;
                    if (liveSegmentationData && video.videoWidth > 0) {
                        // Map canvas 'x' coordinate to the full video width for segmentation data
                        const segX = Math.floor(x * (video.videoWidth / dimensions.width));
                        const segY = Math.floor(y * (video.videoHeight / dimensions.height));
                    ...
                    let opacity = isHighlighted ? 1.0 : (isPerson ? 0.4 : 0.2);
                    const baseColor = isPerson ?'0, 255, 17': '255, 255, 255' ;
    ...
    return (
        <div style={{ backgroundColor: 'black', width: '100vw', height: '100vh', overflow: 'hidden', transform: 'scaleX(-1)' }}>
            {errorMessage && (
